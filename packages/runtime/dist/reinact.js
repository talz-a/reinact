...
class ArrayWithOrignialIndicies {
    #array = [];
    #originalIndices = [];
    #equalsFn
    ...
    orignialIndexAt(index) {
        return this.#originalIndices[index];
    }
    noopItem(index) {
        return {
            op: ARRAY_DIFF_OP.NOOP,
            originalIndex: this.orignialIndexAt(index),
            index,
            item: this.#array[index],
        }
    }
    ...
    moveItem(item, toIndex) {
        ...
        const operation = {
            ...
            originalIndex: this.orignialIndexAt(fromIndex),
            from: fromIndex,
            index: toIndex,
        ...
        const [orginalIndex] = this.#originalIndices.splice(fromIndex, 1);
        this.#originalIndices.splice(toIndex, 0, orginalIndex);
        return operation;
    }
...
function arraysDiffSequence(oldArray, newArray, equalsFn = (a, b) => a === b) {
    const sequence = [];
    const array = new ArrayWithOrignialIndicies(oldArray, equalsFn);
...
function patchStyles(el, oldStyles = {}, newStyle = {}) {
    const { added, removed, updated } = objectsDiff(oldStyles, newStyle);
